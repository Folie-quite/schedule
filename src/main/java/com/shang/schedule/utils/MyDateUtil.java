...
public class MyDateUtil{  
    ...
    /** 
     * 获取某一年各星期的始终时间 
     * 实例：getWeekList(2016)，2016-12-26至2017-01-01 
     * @param year 年份
     * @return 
     */    
    public static HashMap<Integer,String> getWeekMapOfYear(int year) {    
    	HashMap<Integer,String> map = new LinkedHashMap<Integer,String>();    
    	Calendar c = new GregorianCalendar();    
    	c.set(year, Calendar.DECEMBER, 31, 23, 59, 59);    
    	int count = getWeekthOfYear(c.getTime());    
    	...
    	for (int i = 1; i <= count; i++) {    
    		dayOfWeekStart = sdf.format(getFirstDayOfWeek(year, i));    
    		dayOfWeekEnd = sdf.format(getLastDayOfWeek(year, i));    
    		map.put(Integer.valueOf(i), dayOfWeekStart + " 00:00:00至" + dayOfWeekEnd + " 23:59:59");    
    	}    
    	return map;    
    ...
    public static boolean validateIsDate(String strTime) {  
        ...
        if (date != null) {  
            System.out.println("生成的日期:"+MyDateUtil.parseDateToStr(date, MyDateUtil.DATE_TIME_FORMAT_YYYY_MM_DD_HH_MI_SS, "--null--"));  
            return true;  
        }  
        return false;  
    }  
	...
	public static void main(String [] args){  
        try {
            Date yesterday = getYestardayDate();
